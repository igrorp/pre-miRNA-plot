...
class Parser(basic.Parser):
    ...
    # dialects = {
    #     'simple': Dialect('.:-', '(<', '>)', '{[', ']}'),
    #     'generic': Dialect('.:-', '(<', '>)', '{[' + string.uppercase, string.lowercase[::-1] + ']}'),
    #     'rfam': Dialect('.;', '(<[{', '}]>)', string.uppercase, string.lowercase[::-1]),
    # }

    dialects = {
        'simple': Dialect('.:-', '(<', '>)', '{[', ']}'),
        'generic': Dialect('.:-', '(<', '>)', '{['.upper(), ']}'.lower()[::-1]),
        #'rfam': Dialect('.;'.upper(), '(<[{', '}]>)'.lower()[::-1]),
    }
